...
def run():
    ...
    if args["action"] == "sync":
        ...
        if media_type is None or media_type == "None":
            media_type = kodiUtilities.getMediaType()
            xbmc.log(
                "Got the mediatype from selected item: %s" % media_type, xbmc.LOGINFO
            )
        ...
        if media_type is None:
            ...
            if "dbid" in data:
                logger.debug(
                    "Manual %s of library '%s' with an ID of '%s'."
                ...
                if (
                    utilities.isShow(media_type)
                    or utilities.isSeason(media_type)
                    or utilities.isEpisode(media_type)
                ):
            ...
            logger.debug(
                "Manual %s of '%s' is unsupported." % (args["action"], media_type)
            )
                ...
                result = kodiUtilities.kodiJsonRequest(
                    {
                        ...
                        "params": {
                            "properties": ["title", "imdbnumber", "uniqueid", "year"]
                        },
                        "id": 0,
                    }
                )
